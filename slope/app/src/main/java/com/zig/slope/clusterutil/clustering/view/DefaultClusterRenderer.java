...
package com.zig.slope.clusterutil.clustering.view;
...
import com.zig.slope.clusterutil.MarkerManager;
import com.zig.slope.clusterutil.clustering.Cluster;
import com.zig.slope.clusterutil.clustering.ClusterItem;
import com.zig.slope.clusterutil.clustering.ClusterManager;
import com.zig.slope.clusterutil.projection.Point;
import com.zig.slope.clusterutil.projection.SphericalMercatorProjection;
import com.zig.slope.clusterutil.ui.IconGenerator;
import com.zig.slope.clusterutil.ui.SquareTextView;
...
import static com.zig.slope.clusterutil.clustering.algo.NonHierarchicalDistanceBasedAlgorithm.MAX_DISTANCE_AT_ZOOM;
...
public class DefaultClusterRenderer<T extends ClusterItem> implements ClusterRenderer<T> {
    ...
    private class RenderTask implements Runnable {
        ...
        @SuppressLint("NewApi")
        public void run() {
            Log.i("zxx", "runNewApi: clusters==="+clusters.size());
...
//            }
            ...
            Log.i("zxx", "run: clusters==="+clusters.size());
            for (Cluster<T> c : clusters) {
                boolean onScreen = visibleBounds.contains(c.getPosition());
                Log.i("zxx", "run: onScreen==="+onScreen);
                if(onScreen){
                    if (zoomingIn && SHOULD_ANIMATE) {
                        Log.i("zxx", "run: zoomingIn && SHOULD_ANIMATE===");
                        Point point = mSphericalMercatorProjection.toPoint(c.getPosition());
                        Point closest = findClosestCluster(existingClustersOnScreen, point);
                        if (closest != null) {
                            Log.i("zxx", "run: closest != null===");
                            LatLng animateTo = mSphericalMercatorProjection.toLatLng(closest);
                            markerModifier.add(true, new CreateMarkerTask(c, newMarkers, animateTo));
                        } else {
                            Log.i("zxx", "run: closest == null===");
                        ...
                        Log.i("zxx", "run: zoomingIn && SHOULD_ANIMATE2222");
                        markerModifier.add(onScreen, new CreateMarkerTask(c, newMarkers, null));
                    }
                }
            }
