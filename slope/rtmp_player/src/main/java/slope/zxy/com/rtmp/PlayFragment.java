...
import slope.zxy.com.rtmp.view.AngleView;
...
public class PlayFragment extends Fragment implements TextureView.SurfaceTextureListener, PhotoViewAttacher.OnMatrixChangedListener {
    ...
    public boolean onRecordOrStop() {
        if (!mStreamRender.isRecording()) {
            File f = new File(Constantplay.getsMoviePath(getActivity()));
            f.mkdirs();
...
//                    }
    ...
    public View onCreateView(LayoutInflater inflater, final ViewGroup container,
        ...
        if (!TextUtils.isEmpty(mUrl)) {
            Glide.with(this).load(Constantplay.url2localPosterFile(getActivity(), mUrl)).signature(new StringSignature(UUID.randomUUID().toString())).fitCenter().into(cover);
        }
        return view;
    }
        ...
        mSurfaceView.post(new Runnable() {
            ...
            public void run() {
                if (mWidth != 0 && mHeight != 0) {
                    ...
                    File f = Constantplay.url2localPosterFile(mSurfaceView.getContext(), mUrl);
...
//        fixPlayerRatio(renderView, maxWidth, maxHeight, mWidth, mHeight);
    ...
    protected void startRending(SurfaceTexture surface) {
        mStreamRender = new EasyPlayerClient(getContext(), Constante.KEY, new Surface(surface), mResultReceiver);
