    ...
    private void login() {
        ...
        Thread loginRunnable = new Thread() {
            ...
            public void run() {
                ...
                //判断账号和密码getAccount().equals("zxy") && getPassword().equals("5520")
                if (getAccount()!=null) {
                    showToast("登录成功");

                    loadCheckBoxState();//记录下当前用户记住密码和自动登录的状态;
                    ARouter.getInstance().build("/map/index").withString( "userName" ,getAccount() ).navigation();
                    //startActivity(new Intent(LoginActivity.this, LocationDemo.class));
//                    finish();//关闭页面
                } else {
